...
class LinearSchedule(object):
    ...
    def update(self, t):
        ...
        if t > self.nsteps:
            ...
            interpolation = (t / self.nsteps) * self.eps_begin + (1 - (t / self.nsteps)) * self.eps_end
            self.epsilon = interpolation
...
class LinearExploration(LinearSchedule):
    ...
    def get_action(self, best_action):
        ...
        return np.random.choice([self.env.action_space.sample(), best_action], p=[self.epsilon, 1 - self.epsilon])
...
def test4():
    ...
    """
    env = EnvTest((5, 5, 1))
    exp_strat = LinearExploration(env, 0, 10, 10)
    exp_strat.update(5)
    assert exp_strat.epsilon == 5, "My Test failed"
    print("My Test: ok")
